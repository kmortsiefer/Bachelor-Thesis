...
def is_printing(c):
    ...
    for entry in reply:
        if "BUILDING_FROM_SD" in entry:
            logging.info("Printer is busy right now.")
            return True
...
def setup_logger():
    """
    Some basic settings for the logging library.
    """
...
def main():
    ...
    settings = Settings("printer_settings.json")
    # initialize a connection objection with the specified ip and port of the printer
    connector = Connector(settings.get("ip"), settings.get("port"))
    ...
    while True:
        ...
        while _is_printing:
            ...
            status = format_reply(connector.send_commands(
                ["M27", "M105", "M115", "M119"]))
            ...
            if (status["printer_status"] == "inactive"
                or
