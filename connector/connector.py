...
class Connector:
    ...
    def connect(self):
        logging.info("Connecting to printer [{0}:{1}]...".format(self.ip, self.port))
        self.socket = socket.socket(socket.AF_INET)
        self.socket.settimeout(30)

        try:
            self.socket.connect((self.ip, self.port))
            logging.info("Connected to printer [{0}:{1}].".format(self.ip, self.port))
            return True
        except (TimeoutError, OSError) as e:
            logging.info("Printer [{0}:{1}] is currently offline.".format(self.ip, self.port))
            return False

    # sends a given command to the printer
    # parameter: 'Mxxx' (~ and \r\n will be added: '~Mxxx\r\n')
